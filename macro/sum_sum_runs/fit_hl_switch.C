...
void fit_hl_switch(int group=35){
  ...
  sprintf(fpath,"%d/output_fit_switch_exclude_deadhot_50_150.root",group);
...
//  gStyle->SetOptFit(111);
  sprintf(fpath,"%d/fitting_information_50_150.txt",group);
  ofstream out_txt(fpath);
  for(int i=0;i<128*24*16;i++){
    ...
    if(deadhot_set.find(i)==deadhot_set.end()){      
      ...
      double slope_error = ff->GetParError(1);
      double intercept = ff->GetParameter(0);
      double intercept_error = ff->GetParError(0);
      double chi2 = ff->GetChisquare();
      int ndf = ff->GetNDF();
      hslope->Fill(slope);
      out_txt<<i<<" "<<slope<<" "<<slope_error<<" "
