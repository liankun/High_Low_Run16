...
void fit_hl_all(){
  ...
  char fpath[500];
  TFile* ifile = new TFile("../sum_all_runs_xy_switch.root","READONLY");
  TFile* ofile = new TFile("output_fit_all_runs_50_200_exclude_deadhot.root","RECREATE");

  TH1D* hslope = new TH1D("hslope","fitting slope",500,0,1);
  hslope->GetXaxis()->SetTitle("slope");
  
  TH2D* hslope_chi2 = new TH2D("hslope_chi2","slope vs  chi2",200,0,100,500,0,1);
  hslope_chi2->GetXaxis()->SetTitle("chi2");
  hslope_chi2->GetYaxis()->SetTitle("slope");

  TH3D* hfit_info0 = new TH3D("hfit_info0","fit information arm 0",300,0,150,500,0,1,500,-5,5);
  hfit_info0->GetXaxis()->SetTitle("chi2");
  hfit_info0->GetYaxis()->SetTitle("slope");
  hfit_info0->GetZaxis()->SetTitle("intercept");

  TH3D* hfit_info1 = new TH3D("hfit_info1","fit information arm 1",300,0,150,500,0,1,500,-5,5);
...
//  gStyle->SetOptFit(111);
  ofstream out_txt("fitting_information_9_35.txt");
  for(int i=0;i<128*24*16;i++){
    ...
    TF1* ff = new TF1(hname,fitf,9,35,2); 
