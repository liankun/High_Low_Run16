//calculate the sigma
//I defined not distance
//but difference in percent
#include<fstream>
#include<set>
void calculate_sigma3(){
  ...
  TFile* ofile = new TFile("output_dist3.root","RECREATE");
  for(int i=0;i<128*24*16;i++){
    ...
    TH2D* hdst = new TH2D(hname,hname,120,40-0.5,160-0.5,200,-5,5);
...
//	double dist = fabs(eval_x-x)/sqrt(slope_list[i]*slope_list[i]+1);
	double dist = eval_x-x;
	hdst->Fill(y,dist/fabs(eval_x),content);
      }
    }
    ofile->cd();
    hdst->Write();
